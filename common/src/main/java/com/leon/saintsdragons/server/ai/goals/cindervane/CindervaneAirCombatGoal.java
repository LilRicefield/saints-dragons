...
public class CindervaneAirCombatGoal extends Goal {
    ...
    private void chaseTarget(LivingEntity target) {
        if (shouldDiveChase(target)) {
            DragonAirCombatHelper.chasePredicted(amphithere, target, 3.0D, -0.25D, 0.08D, 0.12D, DIVE_CHASE_SPEED);
            return;
        }
    ...
    private boolean shouldDiveChase(LivingEntity target) {
        if (!amphithere.isFlying() || amphithere.isLanding() || !isTargetAirborne(target)) {
            return false;
        }
        double heightAdvantage = amphithere.getY() - (target.getY() + target.getBbHeight() * 0.5D);
        if (heightAdvantage < DIVE_CHASE_MIN_HEIGHT_ADVANTAGE) {
            return false;
        }
        double dx = target.getX() - amphithere.getX();
        double dz = target.getZ() - amphithere.getZ();
        return dx * dx + dz * dz <= DIVE_CHASE_MAX_HORIZONTAL_DISTANCE * DIVE_CHASE_MAX_HORIZONTAL_DISTANCE;
    }
