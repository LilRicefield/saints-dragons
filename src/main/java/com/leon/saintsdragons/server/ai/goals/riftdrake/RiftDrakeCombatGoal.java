 ...
 */
public class RiftDrakeCombatGoal extends Goal {
    ...
    public RiftDrakeCombatGoal(RiftDrakeEntity drake) {
        this.drake = drake;
    ...
    private void tryPerformAttacks(LivingEntity target) {
        ...
        DragonAbilityType<RiftDrakeEntity, ?> ability = choosePrimaryAttack(target);
        ...
        if (shouldUseClaw()) {
            DragonAbilityType<RiftDrakeEntity, ?> claw = RiftDrakeAbilities.CLAW;
    ...
    private DragonAbilityType<RiftDrakeEntity, ?> choosePrimaryAttack(LivingEntity target) {
        double gap = getGapToTarget(target);
        boolean phaseTwo = drake.isPhaseTwoActive();

        if (gap <= BITE_RANGE) {
            return phaseTwo ? RiftDrakeAbilities.BITE2 : RiftDrakeAbilities.BITE;
        }
        if (gap <= HORN_RANGE) {
            return RiftDrakeAbilities.HORN_GORE;
        }
        return null;
    }
