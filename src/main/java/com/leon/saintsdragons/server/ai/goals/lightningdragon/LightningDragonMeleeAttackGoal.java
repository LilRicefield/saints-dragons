...
public class LightningDragonMeleeAttackGoal extends DragonMeleeAttackGoalBase {
    ...
    private boolean chooseMove(double dist, double angle, boolean los) {
        LightningDragonEntity lightningDragon = (LightningDragonEntity) dragon;
        boolean hornReady = lightningDragon.combatManager.canStart(LightningDragonAbilities.HORN_GORE);
        boolean biteReady = lightningDragon.combatManager.canStart(LightningDragonAbilities.BITE);
    ...
    private void triggerAbility(Move m) {
        LightningDragonEntity lightningDragon = (LightningDragonEntity) dragon;
        if (m == Move.HORN) lightningDragon.combatManager.tryUseAbility(LightningDragonAbilities.HORN_GORE);
        else if (m == Move.BITE) lightningDragon.combatManager.tryUseAbility(LightningDragonAbilities.BITE);
    }

    private void setAbilityCooldown(Move m) {
        LightningDragonEntity lightningDragon = (LightningDragonEntity) dragon;
        if (m == Move.HORN) lightningDragon.combatManager.setAbilityCooldown(LightningDragonAbilities.HORN_GORE, 50);
        else if (m == Move.BITE) lightningDragon.combatManager.setAbilityCooldown(LightningDragonAbilities.BITE, 18);
    }
    ...
    protected void onAttackPerformed(LivingEntity target) {
        ...
        if (lightningDragon.canUseAbility()) {
            ...
            if (abilityChance < 0.25f) {
                ...
                lightningDragon.tryActivateAbility(LightningDragonAbilities.HORN_GORE);
            } else if (abilityChance < 0.4f) {
                // 15% chance to use Lightning Bite ability
                lightningDragon.tryActivateAbility(LightningDragonAbilities.BITE);
            }
        }
    }
