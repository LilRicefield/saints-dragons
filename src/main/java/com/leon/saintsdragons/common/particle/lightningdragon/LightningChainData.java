 ...
 */
public record LightningChainData(float size, Vec3 startPos, Vec3 endPos) implements ParticleOptions {
    public static final ParticleOptions.Deserializer<LightningChainData> DESERIALIZER = new ParticleOptions.Deserializer<>() {
        @Override
        public @NotNull LightningChainData fromCommand(@Nonnull ParticleType<LightningChainData> type, @Nonnull StringReader reader) throws CommandSyntaxException {
            reader.expect(' ');
            ...
            return new LightningChainData(size, new Vec3(startX, startY, startZ), new Vec3(endX, endY, endZ));
        }

        @Override
        public @NotNull LightningChainData fromNetwork(@Nonnull ParticleType<LightningChainData> type, @Nonnull FriendlyByteBuf buf) {
    ...
    public static Codec<LightningChainData> CODEC(@SuppressWarnings("unused") ParticleType<LightningChainData> type) {
        return RecordCodecBuilder.create(b -> b.group(
                Codec.FLOAT.fieldOf("size").forGetter(LightningChainData::size),
                Vec3.CODEC.fieldOf("startPos").forGetter(LightningChainData::startPos),
                Vec3.CODEC.fieldOf("endPos").forGetter(LightningChainData::endPos)
        ).apply(b, LightningChainData::new));
    }
    ...
    @Override
    public @NotNull ParticleType<LightningChainData> getType() {
