 ...
 */
public class RiftDrakeBinderItem extends Item {
    ...
    public RiftDrakeBinderItem(Properties properties) {
        super(properties);
    }
    ...
    public @NotNull InteractionResult interactLivingEntity(@NotNull ItemStack stack,
        ...
        if (!(target instanceof RiftDrakeEntity drake)) {
        ...
        if (!drake.canBeBound()) {
            player.displayClientMessage(
                    Component.translatable("saintsdragons.message.rift_drake_cannot_be_captured"),
                    true);
            return InteractionResult.FAIL;
        }
    ...
    private ItemStack captureDrake(ItemStack stack, RiftDrakeEntity drake, Player player) {
        ...
        player.displayClientMessage(
                Component.translatable("saintsdragons.message.rift_drake_captured", drake.getName().getString()),
                true);
    ...
    private boolean releaseDrake(ItemStack stack, Player player, BlockPos pos) {
        ...
        RiftDrakeEntity newDrake = new RiftDrakeEntity(ModEntities.RIFT_DRAKE.get(), serverLevel);
        ...
        player.displayClientMessage(
                Component.translatable("saintsdragons.message.rift_drake_released", drakeName),
                true);
    ...
    public void appendHoverText(@NotNull ItemStack stack,
        ...
        tooltip.add(Component.translatable("saintsdragons.tooltip.rift_drake_binder.description"));
        if (isBound(stack)) {
            String name = getBoundRiftDrakeName(stack);
            if (name != null && !name.isEmpty()) {
                tooltip.add(Component.translatable("saintsdragons.tooltip.rift_drake_binder.bound", name));
            }
            tooltip.add(Component.translatable("saintsdragons.tooltip.rift_drake_binder.right_click_to_release"));
        } else {
            tooltip.add(Component.translatable("saintsdragons.tooltip.rift_drake_binder.empty"));
            tooltip.add(Component.translatable("saintsdragons.tooltip.rift_drake_binder.right_click_rift_drake_to_bind"));
        }
    }
